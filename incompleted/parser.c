...
void compileBlockFunction(void)
{
  Object *constObj;
  ConstantValue *constValue;
  ...
  if (lookAhead->tokenType == KW_CONST)
  {
    ...
    compileBlockFunction2();
  }
  else
    compileBlockFunction2();
}

void compileBlockFunction2(void)
{
  Object *typeObj;
  Type *actualType;
  ...
  if (lookAhead->tokenType == KW_TYPE)
  {
    ...
    compileBlockFunction3();
  }
  else
    compileBlockFunction3();
}

void compileBlockFunction3(void)
{
  Object *varObj;
  Type *varType;
  ...
  if (lookAhead->tokenType == KW_VAR)
  {
    ...
    compileBlockFunction4();
  }
  else
    compileBlockFunction4();
}

void compileBlockFunction4(void)
{
  compileSubDecls();
  compileBlockFunction5();
}

void compileBlockFunction5(void)
{
  eat(KW_BEGIN);
  compileStatementsFunction();
  eat(KW_END);
}
...
ConstantValue *compileUnsignedConstant(void)
{
  ...
  case TK_FLOAT:
    eat(TK_FLOAT);
    constValue = makeFLoatConstant(currentToken->value1);
    break;
  case TK_IDENT:
    eat(TK_IDENT);
...
void compileStatementsFunction(void)
{
  int count = 0;
  int i = 0;
...
Type *compileIndexes(Type *arrayType)
{
  ...
  while (lookAhead->tokenType == SB_LSEL)
  {
    ...
    {
      error(ERR_INDEX_FLOAT_ERROR, lookAhead->lineNo, lookAhead->colNo);
    }
    eat(SB_RSEL);
